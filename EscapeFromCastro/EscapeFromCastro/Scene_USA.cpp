...
Scene_USA::Scene_USA(GameEngine* gameEngine, const std::string& levelPath)
    ...
    loadLevel(levelPath);
    loadSpawnPreset("../assets/spawnPresets.txt", spawnPresetId, "USA");
    init();
}
...
void Scene_USA::sMovement(sf::Time dt) {
    ...
    for (auto& e : m_entityManager.getEntities("enemyPolice")) {
        ...
        if (e->hasComponent<CTransform>()) {
            ...
            for (auto& b : m_entityManager.getEntities("enemyDune")) {
                auto overlap = Physics::getOverlapEntity(e, b, "dune");
                if (overlap.x > 0 and overlap.y > 0) {
                    isPoliceObstructed = true;
                }
            }

            if (tfmP.pos.x < tfm.pos.x && !isPoliceObstructed)
                tfm.pos.y += (tfm.vel.y + 60) * eDirection.y * dt.asSeconds();

            tfm.pos.x -= tfm.vel.x * dt.asSeconds();
        }
    }
    for (auto& e : m_entityManager.getEntities("enemyFloridaMan")) {
        bool isFManObstructed = false;
        if (e->hasComponent<CTransform>()) {
            auto& tfm = e->getComponent<CTransform>();
            auto& tfmP = m_player->getComponent<CTransform>();
            ...
            for (auto& b : m_entityManager.getEntities("enemyDune")) {
                auto overlap = Physics::getOverlapEntity(e, b, "dune");
                if (overlap.x > 0 and overlap.y > 0) {
                    isFManObstructed = true;
                }
            }

            if (tfmP.pos.x < tfm.pos.x && !isFManObstructed)
                tfm.pos.y += (tfm.vel.y + 200) * eDirection.y * dt.asSeconds();
    ...
    for (auto e : m_entityManager.getEntities("enemyPolice")) {
        ...
        if (e->hasComponent<CAnimation>()) {
            ...
            if (!anim.animation.m_isRepeating && anim.animation.isLastFrame())
            {
                ...
                e->addComponent<CAnimation>(Assets::getInstance().getAnimation("Police_Dead"));
            }
            anim.animation.update(dt);
        }
    }
...
void Scene_USA::spawnFloridaMan(sf::Vector2f pos) {
    auto floridaMan = m_entityManager.addEntity("enemyFloridaMan");

    floridaMan->addComponent<CTransform>(pos, sf::Vector2f{ m_usaConfig.enemySpeed, 0.f });
    auto& sprite = floridaMan->addComponent<CSprite>(Assets::getInstance().getTexture("FloridaMan")).sprite;
    floridaMan->addComponent<CAnimation>(Assets::getInstance().getAnimation("FloridaMan_Left"));
    sprite.setTexture(Assets::getInstance().getTexture("FloridaMan"));
    auto spriteSize = sprite.getLocalBounds().getSize();
    floridaMan->addComponent<CBoundingBox>(spriteSize);

    floridaMan->addComponent<CType>().floridaMan = true;
    floridaMan->addComponent<CType>().entity = true;
}
void Scene_USA::spawnPolice(sf::Vector2f pos) {
    auto police = m_entityManager.addEntity("enemyPolice");

    police->addComponent<CTransform>(pos, sf::Vector2f{ m_usaConfig.enemySpeed, 0.f });
    auto& sprite = police->addComponent<CSprite>(Assets::getInstance().getTexture("Police")).sprite;
    police->addComponent<CAnimation>(Assets::getInstance().getAnimation("Police_Left"));
    sprite.setTexture(Assets::getInstance().getTexture("Police"));
    auto spriteSize = sprite.getLocalBounds().getSize();
    police->addComponent<CBoundingBox>(spriteSize);

    police->addComponent<CLife>(5);
    police->addComponent<CType>().police = true;
    police->addComponent<CType>().entity = true;

}
void Scene_USA::spawnDune() {

    for (const auto& pos : duneSpawns) {
        auto dune = m_entityManager.addEntity("enemyDune");

        dune->addComponent<CTransform>(pos, sf::Vector2f{ m_usaConfig.enemySpeed, 0.f });
        auto& sprite = dune->addComponent<CSprite>().sprite;
        auto& spriteName = dune->addComponent<CSprite>(Assets::getInstance().getTexture("Dune")).sprite;
        sprite.setTexture(Assets::getInstance().getTexture("Dune"));
        auto spriteSize = sprite.getLocalBounds().getSize();

        dune->addComponent<CBoundingBox>(spriteSize);
        dune->addComponent<CType>().dune = true;
        dune->addComponent<CType>().entity = true;
        ...
        waterAnim->addComponent<CTransform>(sf::Vector2f(pos.x, pos.y + (spriteSize.y / 1.35)), sf::Vector2f{ m_usaConfig.enemySpeed, 0.f });
...
void Scene_USA::spawnShark(sf::Vector2f pos) {
    ...
    shark->addComponent<CTransform>(pos, sf::Vector2f{ m_usaConfig.enemySpeed, 0.f });
...
void Scene_USA::spawnCoca(sf::Vector2f pos) {
    ...
    coca->addComponent<CTransform>(pos, sf::Vector2f{ m_usaConfig.enemySpeed, 0.f });
    ...
    for (auto e : m_entityManager.getEntities("enemyPolice")) {
        ...
        if (lifespan.has) {
            if (lifespan.remaining < 0 && lifespan.isDying == false && lifespan.isDead == false) {
                lifespan.isDying = true;
                e->getComponent<CTransform>().pos.x -= 19.f;
                e->getComponent<CTransform>().pos.y += 11.f;
                e->removeComponent<CTransform>();

                e->removeComponent<CBoundingBox>();
                e->addComponent<CAnimation>(Assets::getInstance().getAnimation("Police_Death"));
